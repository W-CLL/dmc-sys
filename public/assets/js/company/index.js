define(['jquery', 'bootstrap', 'company', 'addtabs', 'adminlte', 'form'], function ($, undefined, Company, undefined, AdminLTE, Form) {
    var Controller = {
        index: function () {
            //双击重新加载页面
            ...
            $("form.sidebar-form").on('mousedown click', '.menuresult a[data-url]', function () {
                Company.api.addtabs($(this).data("url"));
            });
            ...
            if (Config.referer) {
                if (Config.referer === $(addtabs).attr("url")) {
                    ...
                    //刷新页面后跳到到刷新前的页面
                    Company.api.addtabs(Config.referer);
                }
            }
            ...
            $('#password_save').click(function(){
                Fast.api.open("index/password_save","修改密码")
            });
            $(window).resize();
        ...
        login: function () {
            ...
            if (lastlogin) {
                ...
                $("#profile-img").attr("src", Company.api.cdnurl(lastlogin.avatar));
            ...
            Form.api.bindevent($("#login-form"), function (data) {
                ...
                location.href = Company.api.fixurl(data.url);
            }, function (data) {
