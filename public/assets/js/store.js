define(['fast', 'template', 'moment'], function (Fast, Template, Moment) {
    var Store = {
        ...
        init: function () {
            ...
            $(document).on('click', '.btn-dialog,.dialogit', function (e) {
                ...
                var url = Store.api.replaceids(that, $(that).data("url") || $(that).attr('href'));
                var title = $(that).attr("title") || $(that).data("title") || $(that).data('original-title');
                var button = Store.api.gettablecolumnbutton(options);
                if (button && typeof button.callback === 'function') {
                    options.callback = button.callback;
                }
                if (typeof options.confirm !== 'undefined') {
                    Layer.confirm(options.confirm, function (index) {
                        Store.api.open(url, title, options);
                        Layer.close(index);
                    });
                } else {
                    window[$(that).data("window") || 'self'].Store.api.open(url, title, options);
                }
                return false;
            });
            ...
            $(document).on('click', '.btn-addtabs,.addtabsit', function (e) {
                ...
                if (typeof options.confirm !== 'undefined') {
                    Layer.confirm(options.confirm, function (index) {
                        Store.api.addtabs(url, title, icon);
                        Layer.close(index);
                    });
                } else {
                    Store.api.addtabs(url, title, icon);
            ...
            $(document).on('click', '.btn-ajax,.ajaxit', function (e) {
                ...
                options.url = Store.api.replaceids(this, options.url);
                ...
                if (typeof options.confirm !== 'undefined') {
                    Layer.confirm(options.confirm, function (index) {
                        Store.api.ajax(options, success, error);
                        Layer.close(index);
                    });
                } else {
                    Store.api.ajax(options, success, error);
    ...
    Store.api = $.extend(Fast.api, Store.api);
    //将Template渲染至全局,以便于在子框架中调用
    window.Template = Template;
    //将Moment渲染至全局,以便于在子框架中调用
    window.Moment = Moment;
    //将Store渲染至全局,以便于在子框架中调用
    window.Store = Store;
