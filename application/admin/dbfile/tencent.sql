...
-- 转账失败记录表
CREATE TABLE IF NOT EXISTS `transfer_failure`
(
    `id`                  int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `type`                tinyint(1)       NOT NULL DEFAULT '1' COMMENT '失败类型：1-更新订单号失败 2-插入记录失败',
    `transfer_records_id` int(11)          NOT NULL DEFAULT '0' COMMENT '转账记录ID',
    `transfer_table`      varchar(50)      NOT NULL DEFAULT '' COMMENT '转账表名：tencent_transfer_log 或 tencent_wallet_transfer_log',
    `order_uid`           varchar(100)     NOT NULL DEFAULT '' COMMENT '订单号',
    `record`              text COMMENT '接口返回记录（JSON）',
    `error`               text COMMENT '错误信息',
    `data`                text COMMENT '原始数据（JSON）',
    `retry_count`         int(11)          NOT NULL DEFAULT '0' COMMENT '重试次数',
    `status`              tinyint(1)       NOT NULL DEFAULT '0' COMMENT '状态：0-待重试 1-成功 2-失败',
    `remark`              varchar(500)     NOT NULL DEFAULT '' COMMENT '备注',
    `create_time`         int(11)          NOT NULL DEFAULT '0' COMMENT '创建时间',
    `update_time`         int(11)          NOT NULL DEFAULT '0' COMMENT '更新时间',
    PRIMARY KEY (`id`),
    KEY `idx_transfer_records_id` (`transfer_records_id`),
    KEY `idx_order_uid` (`order_uid`),
    KEY `idx_status` (`status`),
    KEY `idx_create_time` (`create_time`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4 COMMENT ='转账失败记录表';
