...
class Transfer extends Store
{
    public function _initialize()
    {
        ...
        $this->TencentWallteModel = new TencentStore();
    ...
    public function Transfer($ids = null){
        ...
        $store_info = $this->TencentWallteModel->where('store_id',$this->auth->id)->lock(true)->find();
        if(empty($store_info)){
        ...
        $can_transfer_out_money = $res['data']['recommend_amount'] / 100 ;
        ...
        if ($this->request->isPost()) {
            ...
            $wallet_info = $this->checkParam($post, $account, $store_info, $can_transfer_out_money);
            Db::startTrans();
            try{
        ...
        $this->view->assign('canTransferOutMoney',$can_transfer_out_money);
        $this->view->assign('account', $account);
        $this->view->assign('storeInfo', $store_info);
        return $this->fetch();
    }

    private function checkParam($post, $account, $store_info, $can_transfer_out_money){
    ...
    private function deductMoney($store,$transfer_data){
        if($transfer_data['transfer_direction'] == 1){
            if($transfer_data['account_type'] == 1){
                $res = $this->TencentWallteModel->where([
                    'id'=>['=',$store['id']],
    ...
    private function createTransactionLog($transfer_id, $insert_data, $store_info){
        ...
        try {
            ...
            if($insert_data['transfer_direction'] == 1){
                ...
                $sql = $this->TencentWallteModel->where([
                    'id'=>['=',$store_info['id']]
                ])
