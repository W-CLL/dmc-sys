...
CREATE TABLE `fa_share_wallet_transfer_log` (
                                                ...
                                                `discount_percentage` decimal(5,2) NOT NULL DEFAULT '0.00' COMMENT '折扣百分比',
                                                PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='共享钱包转账记录表';
--2024.09.13
--2024.9.13 已处理
ALTER TABLE fa_store_money_log ADD COLUMN `swtl_id` int(11) DEFAULT NULL COMMENT '共享钱包转账记录id';
ALTER TABLE fa_store_money_log
    MODIFY COLUMN `type` int(11) COMMENT '类型：1为总后台增加余额，2为总后台扣款，3回单充值，4千川转入，5千川转出,6授信额度充值，7子账户充值，8共享钱包转入，9共享钱包转出';
