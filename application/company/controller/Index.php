...
//            $token = $this->request->post('__token__');
            $rule = [
                'company_name'  => 'require|length:3,30',
...
//                '__token__' => $token,
            ...
            $validate = new Validate($rule, [], ['company_name' => "账户名", 'password' => __('Password'), 'captcha' => __('Captcha')]);
            $result = $validate->check($data);
            if (!$result) {
                $this->error($validate->getError(), $url, ['token' => $this->request->token()]);
            }

            $result = $this->auth->login($company_name, $password, $keeplogin ? $keeyloginhours * 3600 : 0);
            if ($result === true) {
                Hook::listen("admin_login_after", $this->request);
                $this->success(__('Login successful'), $url, ['url' => $url, 'id' => $this->auth->id, 'company_name' => $company_name, 'avatar' => $this->auth->avatar]);
            } else {
    ...
    public function password_save(){
        if ($this->request->isPost()){
            ...
            if (Db::name("company")->where("id",$this->auth->id)->update($params)){
                $this->success("修改成功");
            }
            $this->error("修改失败");
        }
