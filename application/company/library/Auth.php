...
class Auth extends \fast\Auth
{
     ...
     * 管理员登录
     *
     * @param string $company_name 用户名
     * @param string $password 密码
     * @param int    $keeptime 有效时长
     * @return  boolean
     */
    public function login($company_name, $password, $keeptime = 0)
    {
        $company_data = Db::name("company")->where(['company_name' => $company_name])->find();
        if (!$company_data) {
            $this->setError('用户不存在');
            return false;
        }
        ...
        if (Config::get('fastadmin.login_failure_retry') && $company_data['loginfailure'] >= 10 && time() - $company_data['update_time'] < 86400) {
            $this->setError('请在1天后重试');
            return false;
        }
        if ($company_data['password'] != $this->getEncryptPassword($password, $company_data['salt'])) {
            Db::name("company")->where(['id'=>$company_data['id']])->setInc('loginfailure');
            $this->setError('密码不正确');
            return false;
        }
        $company_data['loginfailure'] = 0;
        $company_data['login_time'] = time();
        $company_data['loginip'] = request()->ip();
        $company_data['token'] = Random::uuid();
        Db::name("company")->where(['id'=>$company_data['id']])->update($company_data);
        Session::set("company", $company_data);
        Session::set("company.safecode", $this->getEncryptSafecode($company_data));
        $this->keeplogin($company_data, $keeptime);
        return true;
    }
    ...
    public function logout()
    {
        ...
        Db::name("company")->where(['id'=>$this->id])->update(['token'=>'']);
        $this->logined = false; //重置登录状态
        Session::delete("company");
        Cookie::delete("company_keeplogin");
        setcookie('fastcompany_userinfo', '', $_SERVER['REQUEST_TIME'] - 3600, rtrim(url("/" . request()->module(), '', false), '/'));
        return true;
    }
    ...
    {
        $keeplogin = Cookie::get('company_keeplogin');
        if (!$keeplogin) {
            return false;
        }
        ...
        if ($id && $keeptime && $expiretime && $key && $expiretime > time()) {
            ...
            //token有变更
            if ($key != $this->getKeeploginKey($company, $keeptime, $expiretime)) {
                return false;
            }
            $ip = request()->ip();
            //IP有变动
            if ($company['loginip'] != $ip) {
                return false;
            }
            Session::set("company", $company);
            Session::set("company.safecode", $this->getEncryptSafecode($company));
            //刷新自动登录的时效
            $this->keeplogin($company, $keeptime);
            return true;
        } else {
            return false;
        }
     ...
     * @return  boolean
     */
    protected function keeplogin($company, $keeptime = 0)
    {
        if ($keeptime) {
            $expiretime = time() + $keeptime;
            $key = $this->getKeeploginKey($company, $keeptime, $expiretime);
            Cookie::set('company_keeplogin', implode('|', [$company['id'], $keeptime, $expiretime, $key]), $keeptime);
            return true;
        }
        return false;
    }
    ...
    {
        return md5(md5($params['company_name']) . md5(substr($params['password'], 0, 6)) . config('token.key'));
    }
    ...
    public function check_rule($name, $relation = 'or', $mode = 'url')
    {
        ...
        $rules = Db::name("company_group")->where("id",$this->group_id)->value("rules");
        // 获取用户需要验证的所有有效规则列表
        $rulelist = Db::name("company_rule")->where(['id'=>['in',$rules]])->column("name");
    ...
    public function isLogin()
    {
        ...
        $my = Db::name("company")->where(['id'=>$company['id']])->find();
        if (!$my) {
            return false;
        }
        //校验安全码，可用于判断关键信息发生了变更需要重新登录
        if (!isset($company['safecode']) || $this->getEncryptSafecode($my) !== $company['safecode']) {
            $this->logout();
            return false;
        }
        $this->logined = true;
    ...
    public function getUserInfo($uid = null)
    {
        ...
        return $uid != $this->id ? Db::name("company")->where('id',intval($uid))->find() : Session::get('company');
    }
    ...
    {
        $company = Session::get('company');
        $groupList = Db::name("company_group")->where(['id'=>$company['group_id'],'status' => 'normal'])->find();
    ...
    {
        // 边栏开始
        Hook::listen("company_sidebar_begin", $params);
        ...
        $userRule = [];
        $rules = Db::name("company_group")->where("id",$this->group_id)->value('rules');
        $userRule = Db::name("company_rule")->where(['id'=>['in',$rules]])->column("name");

        $selected = $referer = [];
        $refererUrl = Session::get('company_referer');

        // 必须将结果集转换为数组
        $ruleList = Db::name("company_rule")->where('status', 'normal')
            ->where('ismenu', 1)
            ->order('weigh', 'desc')
            ->select();
        $indexRuleList = Db::name("company_rule")->where('status', 'normal')
            ->where('ismenu', 0)
