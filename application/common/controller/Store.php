...
class Store extends Controller
{
    ...
    public function _initialize()
    {
        ...
        if (!$this->auth->match($this->noNeedLogin)) {
            ...
            if (!$this->auth->isLogin()) {
                Hook::listen('store_nologin', $this);
                $url = Session::get('store_referer');
                $url = $url ? $url : $this->request->url();

                if (in_array($this->request->pathinfo(), ['/', 'index/index'])) {
                    $this->redirect('index/login', [], 302, ['store_referer' => $url]);
                    exit;
                }
            ...
            if (!$this->auth->match($this->noNeedRight)) {
                ...
                // 判断控制器和方法是否有对应权限
                if (!$this->auth->check_rule($path)) {
                    Hook::listen('store_nopermission', $this);
        ...
        if (!$this->request->isPost() && !IS_AJAX && !IS_ADDTABS && !IS_DIALOG && input("ref") == 'addtabs') {
            ...
            $this->redirect('index/index', [], 302, ['store_referer' => $url]);
            exit;
        }
        ...
        $config = [
            ...
            'jsname'         => 'store/' . str_replace('.', '/', $controllername),
            'moduleurl'      => rtrim(url("/{$modulename}", '', false), '/'),
            'language'       => $lang,
            'store_referer'        => Session::get("store_referer")
        ];
        ...
        //渲染管理员对象
        $this->assign('store', Session::get('store'));
    ...
    protected function screen_filter($filter){
        $aliasName = '';
        $index = 0;
        $name = '';
        $alias = [];
        ...
        foreach ($filter as $k => $v) {
            if ($k == "create_time"){
                ...
                $where["create_time"] = ["between",[$startDateTimestamp , $endDateTimestamp]];
