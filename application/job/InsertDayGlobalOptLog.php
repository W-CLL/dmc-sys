...
use app\admin\model\QcGlobalObjOptLog;
...
class InsertDayGlobalOptLog
{
    ...
    protected function doJob($params, $queueData)
    {
        ...
        if($resData['code'] == 0) {
            ...
            if ($totalNumber <= 20 && $totalNumber > 0) {
                ...
                //从第二页开始用队列进行写入
                $queue->addQueue('插入计划操作日志', 'app\job\InsertGlobalObjOptLog', 'insertGlobalObjOptLog', $queueData);
            }
            return true;
        } else {
            throw new Exception($resData['message']);
    ...
    {
        $insertData = [];
        $objOptLogModel = new QcGlobalObjOptLog();
